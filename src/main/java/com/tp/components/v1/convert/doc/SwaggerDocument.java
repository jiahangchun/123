...
public class SwaggerDocument extends MarkupComponent<SwaggerDocument.Parameters> {
    ...
    private void buildDocumentResponseBody(
                    ...
                    resultData.stream().collect(
                            Collectors.toMap(
                                    ...
                                    x -> {
                                        ...
                                        if (JMCommonUtil.isEmpty(type)) {
                                            return "";
                                        }
                                        String defaultValue = JMCommonUtil.getDefaultValue(type);
                                        return defaultValue;
                                    },
                                    (k1, k2) -> k2));
            Object refDataStr = JMCommonUtil.generateObjectByField(refData);
        ...
        String objStr = JMCommonUtil.generateObjectByFieldToStr(map);
        ...
        if (JMCommonUtil.isNotEmpty(type)) {
            switch (type) {
                case "array":
                    ...
                    if (JMCommonUtil.isEmpty(defaultValue)) {
                        defaultValue = "";
                    }
                    for (int i = 0; i < 1; i++) {
    ...
    private void buildDocumentResponseParam(MarkupDocBuilder markupDocBuilder,
        ...
        if (JMCommonUtil.isEmpty(requestResultBos)) {
            return;
        }
        ...
        for (RequestResultBo requestResultBo : requestResultBos) {
            ...
            if (JMCommonUtil.isNotEmpty(ref)) {
                ...
                if (JMCommonUtil.isNotEmpty(resultDataList)) {
    ...
    private void buildDocumentRequestBody(MarkupDocBuilder markupDocBuilder,
                                          ...
                                          Map<String, List<ResultData>> definitionVoMap) {
        if (JMCommonUtil.isEmpty(requestParamVos)) {
            return;
        }
        //首先对参数中引用的部分设置默认的值
    ...
    private String formRequestBody(List<RequestParamBo> requestParamVos) {
        ...
        return JMCommonUtil.generateObjectByFieldToStr(map);
    }
    ...
    private void buildDocumentRequestParam(
        ...
        for (RequestParamBo requestParamBo : requestParamVos) {
            ...
            if (JMCommonUtil.isEmpty(required)) {
    ...
    private void buildDocumentMethod(MarkupDocBuilder markupDocBuilder, HttpMethod method) {
        String methodStr = "";
        if (JMCommonUtil.isNotEmpty(method)) {
