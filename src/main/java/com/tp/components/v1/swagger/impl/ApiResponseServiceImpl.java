package com.tp.components.v1.swagger.impl;

import com.tp.components.v1.common.JMCommonUtil;
import com.tp.components.v1.common.OkHttpRequestUtils;
import com.tp.components.v1.convert.data.format.SwaggerDataFormat;
import com.tp.components.v1.swagger.ApiResponseService;
import com.tp.components.v1.swagger.dto.OpenApi;
import com.tp.components.v1.swagger.dto.Parameter;
import com.tp.components.v1.swagger.dto.ResultData;
import com.tp.components.v1.swagger.dto.SwaggerApiListDto;
import com.tp.components.v1.swagger.parm.SwaggerApiListParam;
import com.tp.components.v1.swagger.vo.*;
...
public class ApiResponseServiceImpl implements ApiResponseService {
    ...
    private List<SwaggerApiListDto> queryAllList() {
        ...
        if (JMCommonUtil.isEmpty(swaggerApiListDtoList)) {
    ...
    private void loadDefinitionListToLocalCache(Map<String, List<ResultData>> resultMap) {
        if (JMCommonUtil.isEmpty(resultMap)) {
            return;
        }
    ...
    private void loadApiToLocalCache(List<SwaggerApiListDto> swaggerApiList) {
        if (JMCommonUtil.isEmpty(swaggerApiList)) {
            return;
        }
    ...
    public Boolean filter(SwaggerApiListVo listVo, SwaggerApiListParam swaggerApiListParam) {
        if (JMCommonUtil.isEmpty(listVo) || JMCommonUtil.isEmpty(swaggerApiListParam)) {
            return true;
        }
        ...
        //没有任何筛选条件
        if (JMCommonUtil.isEmpty(tag) &&
                JMCommonUtil.isEmpty(key)
                && JMCommonUtil.isEmpty(url)
                && JMCommonUtil.isEmpty(method)
                && JMCommonUtil.isEmpty(description)) {
            return true;
        }
        if (JMCommonUtil.isNotEmpty(tag) && !listVo.getTagStr().contains(tag)) {
            return false;
        }
        if (JMCommonUtil.isNotEmpty(key) && !Objects.equals(listVo.getKey(), key)) {
            return false;
        }
        if (JMCommonUtil.isNotEmpty(url) && !listVo.getUrl().contains(url)) {
            return false;
        }
        if (JMCommonUtil.isNotEmpty(method) && !Objects.equals(listVo.getMethod(), method)) {
            return false;
        }
        if (JMCommonUtil.isNotEmpty(description) && !listVo.getDescription().contains(description)) {
            return false;
        }
        return true;
    }
    ...
    public SwaggerApiDetailVo getSwaggerVo(String key) {
        if (JMCommonUtil.isEmpty(key)) {
            throw new RuntimeException("can not found this interface");
        }
        SwaggerApiListDto swaggerApiListDto = swaggerApiListDtoMap.get(key);
        if (JMCommonUtil.isEmpty(swaggerApiListDto)) {
            throw new RuntimeException("can not found this interface");
        }
        ...
        if (JMCommonUtil.isNotEmpty(parameterList)) {
            List<RequestParamVo> requestParamVos = parameterList.stream().map(x -> {
                ...
                if (JMCommonUtil.isNotEmpty(type)) {
                    requestParamVo.setType(type);
                    if (JMCommonUtil.isNotEmpty(defaultValue)) {
                ...
                if (JMCommonUtil.isNotEmpty(requestParamVo.getRef())
                        && JMCommonUtil.isEmpty(requestParamVo.getDefaultValue())) {
                    requestParamVo.setDefaultValue("{}");
                }
        ...
        if (JMCommonUtil.isNotEmpty(ref)) {
            ...
            if (JMCommonUtil.isNotEmpty(result)) {
                ref = result;
            }
            ...
            if (JMCommonUtil.isNotEmpty(relDataList)) {
                ...
                for (ResultData sample : relDataList) {
                    RequestResultVo requestResultVo = JMCommonUtil.copyProperties(sample, RequestResultVo.class);
    ...
    public MockRequestVo mockRequest(SwaggerApiDetailVo swaggerApiDetailVo) {
        if (JMCommonUtil.isEmpty(swaggerApiDetailVo)) {
        ...
        if (JMCommonUtil.isNotEmpty(paths)) {
            for (Map.Entry<String, Object> entry : paths.entrySet()) {
                String key = entry.getKey();
                Object val = entry.getValue();
                if (JMCommonUtil.isEmpty(key) || JMCommonUtil.isEmpty(val)) {
                    continue;
                }
        ...
        try {
            ...
            switch (swaggerApiDetailVo.getMethod()) {
                ...
                case POST:
                    ...
                    String json = "";
                    if (JMCommonUtil.isNotEmpty(params)) {
        ...
        MockRequestVo mockRequestVo = JMCommonUtil.copyProperties(swaggerApiDetailVo, MockRequestVo.class);
    ...
    public static SwaggerApiListVo convert2Vo(SwaggerApiListDto dto) {
        SwaggerApiListVo swaggerApiListVo = JMCommonUtil.copyProperties(dto, SwaggerApiListVo.class);
        swaggerApiListVo.setMethod(dto.getMethod().name());
        List<String> tags = swaggerApiListVo.getTags();
        if (JMCommonUtil.isNotEmpty(tags)) {
    ...
    public static String data(String url) {
        String data = "";
        try {
            data = JMCommonUtil.urlToString(url);
        } catch (Exception e) {
        }
        return data;
    }
